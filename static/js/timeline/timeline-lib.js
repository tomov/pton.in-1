...
links.Timeline.prototype.setRealData = function(data) {
    this.realData = data;
}
...
links.Timeline.prototype.setData = function(data) {
    ...
    if (google && google.visualization &&
        (data instanceof google.visualization.DataTable || 
            data instanceof google.visualization.DataView) ) {
...
links.Timeline.prototype.updateData = function  (index, values) {
    var data = this.data,
        prop;
    if (data instanceof google.visualization.DataView) {
        index = data.getViewRows()[index];
        data = this.realData; // we were in the "filtered" view data (limited by location)
    }
...
links.Timeline.prototype.onMouseDown = function(event) {
    ...
    if (params.addItem) {
        ...
        // Momchil added this here lol... it's to make sure we call onNew even when someone adds an event on the timeline
        this.applyAdd = true;
        // fire an add event.
        // Note that the change can be canceled from within an event listener if
        // this listener calls the method cancelAdd().
