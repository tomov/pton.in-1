...
var htmltooltip={
	...
	showtip:function($, tipindex, e){
        if (current_tip_idx != -1) {
            htmltooltip.hidetip($, current_tip_idx, e);
        }
        current_tip_idx = tipindex;
		var tooltip=this.tooltips[tipindex]
...
// htmltooltip.render() -- it only makes sense to render after the google visualization (i.e. the timeline) is loaded, so we call this in my_timeline instead (Momchil)
